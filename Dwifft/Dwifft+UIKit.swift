    ...
    open var rows : [T] {
        didSet {
            ...
            if (diff.results.count > 0) {
                ...
                let insertionIndexPaths = diff.insertions.map({ IndexPath(row: $0.index, section: self.sectionIndex) })
                let deletionIndexPaths = diff.deletions.map({ IndexPath(row: $0.index, section: self.sectionIndex) })
...
open class CollectionViewDiffCalculator<T: Equatable> {
    ...
    open var rows : [T] {
        didSet {
            ...
            let diff = oldRows.diff(newRows)
            if (diff.results.count > 0) {
                let insertionIndexPaths = diff.insertions.map({ IndexPath(item: $0.index, section: self.sectionIndex) })
                let deletionIndexPaths = diff.deletions.map({ IndexPath(item: $0.index, section: self.sectionIndex) })
